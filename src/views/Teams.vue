					...
					<tr v-for="(item, index) in teams" :key="index">
						<td>
							<div class="item-box item-customer">
								<h5 class="m-0 o-070" v-if="!item.edit">{{item.team_name}}</h5>
                <el-input placeholder="Багийн нэр" v-model="item.name_clone" v-else clearable size="mini"></el-input>
							</div>
						</td>
            <td>
							<div class="item-box item-customer">
								<h5 class="m-0 o-070" v-if="!item.edit">{{item.sort}}</h5>
                <el-input placeholder="Дараалал" v-model="item.sort_clone" v-else clearable size="mini"></el-input>
							</div>
						</td>
						<td align="right">
							<el-button @click="item.edit = true" v-if="!item.edit" type="primary" plain icon="el-icon-edit" size="mini"></el-button>
              <el-button @click="(item.edit = false, item.name_clone = item.team_name, item.sort_clone = item.sort)" v-if="item.edit" type="primary" plain icon="el-icon-close" size="mini"></el-button>
              <el-button @click="update(item)" v-if="item.edit" type="warning" :loading="item.loading" plain icon="el-icon-check" size="mini"></el-button>
							<el-popover
								placement="top-end"
								width="160"
...
export default {
  ...
  created() {
	...
	methods: {
		deletePro(item) {
			this.$axios({
				...
				this.teams.forEach((elm, indx) => {
          elm.seq === item.seq ? this.teams.splice(indx, 1) : null;
        });
			}).catch(err => {
				console.log(err);
			});
    ...
    update(p) {
      if (p.team_name != p.name_clone || p.sort != p.sort_clone) {
        p.loading = true;
        this.$axios({
          method: 'PUT',
          url: '/a1/team/update',
    ...
    getTeams() {
      this.$axios({
        ...
        if (data.data.message === 'Success') {
          ...
          dt.forEach(elm => {
            elm.edit = false;
            elm.name_clone = elm.team_name;
            elm.sort_clone = elm.sort;
            elm.loading = false;
          });
