...
export default {
  ...
  methods: {
    async getCareers() {      
      const data = await this.$useapi('POST', '/v1/team/get-career', { seq: this.id });
      if (data && data.data.data && data.data.data.length > 0) {
        const p = data.data.data[0];
        this.career.seq = this.id;
        this.career.title = p.title;
        this.career.jobType = p.jobType;
        this.career.level = p.levels;
        this.career.salary = p.salary;
        this.requirement = p.requirement;
        this.role = p.roles;
        this.about = p.about;
        this.career.lang = p.lang;
      } else {
        this.$router.push('/career');
      }      
    },
    async saveCareer() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          const data = this.$useapi('PUT', '/a1/team/update-career', { career: this.career});
          if (data) {
            this.$notify({
              title: 'Амжилттай',
