	...
	<div class="page-ecommerce-dashboard scrollable">
        ...
        <el-button type="primary" plain class="addProperty" size="mini" @click="addNewPro">Hэмэх</el-button>
			...
			<table class="table-box card-base card-outline styled striped hover">
				...
				<tbody>
					<tr v-for="(item, index) in professions" :key="index">
						<td>
              ...
              <el-button @click="(item.edit = false, item.pro_name_clone = item.pro_name)" v-if="item.edit" type="primary" plain icon="el-icon-close" size="mini"></el-button>
  ...
  created() {
	...
	methods: {
		deletePro(item) {
			...
			this.$axios({
				method: 'DELETE',
				url: '/a1/delete-profession',
				data: {
					seq: item.seq
				}
			}).then(() => {
				this.professions.forEach((elm, index) => {
					elm.seq === item.seq ? this.professions.splice(index, 1) : null;
				});
			}).catch(err => {
				console.log(err);
			});
